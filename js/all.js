...
function generateBlock(product){
    ...
    form+=' <button id="buy'+product['id']+'" value = "'+product['price'][0]+'" class="buy" onclick="buy('+product["id"]+')">'+((basket['items'].indexOf([product['id'],0])!=-1)?"In the cart":"Buy")+'</button>';
    form+='</div>';
    return form;
}

window.onload = async function(path){
    let href = window.location.href;
    let inds = await fetch("https://my-json-server.typicode.com/ValeryDrozd/Valerydrozd.github.io/categories/"+path).then(res => res.json());
    if(path!='all'){
        inds = inds['values'];
    }
